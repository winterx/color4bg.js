...
import shader from "./shaders/SwirlingCurves.js"

export class SwirlingCurvesBg extends ColorBg {
	constructor(params = {}) {
		super(params, 6)
		this.name = "swirling-curves"
	...
	_initRtt() {
		...
		this.rttProgram = new Program(this.gl, {
			...
			uniforms: {
				u_time: { value: 0 },
				u_resolution: { value: new Vec2(1024, 1024) },
				u_expand: { value: 6.0 },
				u_random: { value: this.rng() },
				u_scale: { value: 8.0 },
				u_density: { value: 1500 },
	...
	update(option, val) {
		switch (option) {
			...
			case "density":
				this.rttProgram.uniforms.u_density.value = parseFloat(val)
				break
